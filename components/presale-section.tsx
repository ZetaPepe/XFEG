...
export function PresaleSection() {
  ...
  const fetchPresaleBalance = async () => {
    ...
    try {
      ...
      const balance = await web3.eth.getBalance(PRESALE_ADDRESS)
      const balanceInOKB = Number.parseFloat(web3.utils.fromWei(balance, "ether"))
      setPresaleBalance(balanceInOKB)
    } catch (error) {
  ...
  useEffect(() => {
    fetchPresaleBalance()
    const balanceInterval = setInterval(fetchPresaleBalance, 30000)
  ...
  const handleMint = async () => {
    ...
    try {
      ...
      setMintSuccess(true)
      setTimeout(fetchPresaleBalance, 5000)
    } catch (error: any) {
  ...
  const PresaleProgress = () => {
    const progressPercentage = Math.min((presaleBalance / PRESALE_GOAL) * 100, 100)
    const remainingOKB = Math.max(PRESALE_GOAL - presaleBalance, 0)
    ...
    return (
      <Card className="border-primary/20 mb-6">
        ...
        <CardContent className="space-y-4">
          ...
          {isLoadingBalance && <div className="text-center text-xs text-muted-foreground">Updating balance...</div>}
        </CardContent>
      </Card>
    )
  }
